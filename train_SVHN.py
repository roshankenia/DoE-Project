...
class SVHNDataset(torch.utils.data.Dataset):
    def __init__(self, xmlroot, imgroot, transforms=None):
        ...
        self.xmls = list(
            sorted(os.listdir(os.path.join(xmlroot, "Annotations"))))
    ...
    def __getitem__(self, idx):
        ...
        imgPath = os.path.join(self.imgroot, fileName)
...
dataset = SVHNDataset(xmlRoot, imgRoot, get_transform(train=True))
dataset_test = SVHNDataset(xmlRoot, imgRoot, get_transform(train=False))
...
torch.save(model, r'./saved_model/SVHN_model_50epoch.pkl')
